...
export const RiskCatalogEditor: React.FC<RiskCatalogEditorProps> = ({ catalog, onUpdateCatalog }) => {
  ...
  const handleRemoveRisk = (id: string) => {
    if (confirm("Tem certeza que deseja excluir este risco do catálogo? Isso não alterará os riscos já salvos nos inventários existentes, mas ele não estará mais disponível para novas seleções.")) {
      onUpdateCatalog(catalog.filter((r) => r.id !== id));
    }
  };

  const handleResetToDefault = () => {
    if (confirm("Deseja redefinir o catálogo para o padrão do sistema? Isso removerá as alterações personalizadas.")) {
